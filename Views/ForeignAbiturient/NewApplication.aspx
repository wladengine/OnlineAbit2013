...
<%@ Page Title="" Language="C#" MasterPageFile="~/Views/ForeignAbiturient/PersonalOffice.Master" Inherits="System.Web.Mvc.ViewPage<ApplicationModel>" %>
...
<script type="text/javascript">
    ...
    function GetFaculties() {
        ...
        $('#Specs').hide();
        $('#Facs').show();
        $.post('/ForeignAbiturient/GetFacs', { studyform: $('#StudyFormId').val(), studybasis: $('#StudyBasisId').val(), entry: $('#Entry').val() }, function (json_data) {
            var options = '';
    ...
    function GetProfessions() {
        ...
        $('#Specs').hide();
        $('#FinishBtn').hide();
        $.post('/ForeignAbiturient/GetProfs', { studyform: $('#StudyFormId').val(), studybasis: $('#StudyBasisId').val(),
    ...
    function GetObrazPrograms() {
        ...
        $.post('/ForeignAbiturient/GetObrazPrograms', { prof: profId, studyform: sfId, studybasis: $('#StudyBasisId').val(), 
            ...
            var options = '';
            if (json_data.NoFree) {
                $('#ObrazProgramsErrors').text('<%= GetGlobalResourceObject("NewApplication", "ErrorHasApplication")%>');
    ...
    function GetSpecializations() {
        ...
        $('#Specs').hide();
        $('#FinishBtn').hide();
        $.post('/ForeignAbiturient/GetSpecializations', { prof: profId, obrazprogram: opId, studyform: $('#StudyFormId').val(), 
            ...
            if (json_data.List.length == 1 && json_data.List[0].Name == 'нет') {
                ...
                if (json_data.NoFree) {
                    $('#ObrazProgramsErrors').text('<%= GetGlobalResourceObject("NewApplication", "ErrorHasApplication")%>').show();
    ...
    function ChangeEType() {
        ...
        if (entry == 3) {
            ...
            $('#Second').hide();
            $('#IsReduced').removeAttr('checked');
            $('#IsReducedHidden').val('0');
            $('#Reduced').hide();
            $('#IsParallel').removeAttr('checked');
    ...
    function ChangeIsSecond() {
        if ($('#IsSecond').is(':checked')) {
            ...
            $('#SBasis').hide();
            $('#StudyBasisId').html('<option value="2"><%= GetGlobalResourceObject("NewApplication", "Select_StudyBasis2")%></option>');
        }
        else {
            $('#IsSecondHidden').val('0');
            if (!$('#IsParallel').is(':checked') && !$('#IsReduced').is(':checked') && $('#Entry').val() != 3)
            {
                $('#StudyBasisId').html('<option value="1"><%= GetGlobalResourceObject("NewApplication", "Select_StudyBasis1")%></option><option value="2"><%= GetGlobalResourceObject("NewApplication", "Select_StudyBasis2")%></option>');
                $('#SBasis').show();
            }
        }
        GetProfessions();
...
<% using (Html.BeginForm("NewApp", "ForeignAbiturient", FormMethod.Post))
   { 
%> 
    ...
    <% if (Model.EntryType != 1)
       { %>
        ...
        <select id="Entry" name="Entry" onchange="ChangeEType()">
            ...
            <option value="3"><%= GetGlobalResourceObject("NewApplication", "Select_FirstCourse")%></option>
        </select>
    <% }
       else
       { %>
        <div class="message info"><%= GetGlobalResourceObject("NewApplication", "HeaderFirstCourse_1")%> <strong><%= GetGlobalResourceObject("NewApplication", "HeaderFirstCourse_2")%></strong></div>
    ...
    <p id="Parallel" style="display:none; border-collapse:collapse;">
        <input type="checkbox" id="IsParallel" name="IsParallel" title="<%= GetGlobalResourceObject("NewApplication", "IsParallel")%>" onclick="ChangeIsParallel()"/><span style="font-size:13px"><%= GetGlobalResourceObject("NewApplication", "IsParallel")%></span><br />
    ...
    <p id="Second" style="display:none; border-collapse:collapse;">
        <input type="checkbox" id="IsSecond" name="IsSecond" title="<%= GetGlobalResourceObject("NewApplication", "IsSecond")%>" onclick="ChangeIsSecond()"/><span style="font-size:13px"><%= GetGlobalResourceObject("NewApplication", "IsSecond")%></span><br />
    ...
    <p id="FinishBtn" style="border-collapse:collapse;">
        <input type="checkbox" name="NeedHostel" title="<%= GetGlobalResourceObject("NewApplication", "chbNeedHostel")%>" /><span style="font-size:13px"><%= GetGlobalResourceObject("NewApplication", "chbNeedHostel")%></span><br /><br />
        <input id="Submit" type="submit" value="<%= GetGlobalResourceObject("NewApplication", "btnCreateApp")%>" class="button button-green"/>
    </p>
